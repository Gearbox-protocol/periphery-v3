...
contract SecuritizeDefaultLiquidityAttachTest is PeripheryAttachTestBase {
    ...
    function setUp() public {
        ...
        address degenNFT = SecuritizeKYCFactory(factory).degenNFT();
        ...
        ERC4626(cUSDC).deposit(1e5, riskCurator);
        vm.stopPrank();
    ...
    function test_open_credit_account_via_securitize_factory() public {
        ...
        calls[1] = MultiCall({
            target: adapter, callData: abi.encodeCall(IERC4626Adapter.redeem, (50_000e6, address(0), address(0)))
        });
        calls[2] = MultiCall({
            target: creditFacade,
            callData: abi.encodeCall(ICreditFacadeV3Multicall.withdrawCollateral, (USDC, 50_000e6, investor))
        });
        calls[3] = MultiCall({
            target: creditFacade,
